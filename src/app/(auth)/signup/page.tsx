...
export default function SignUpPage() {
  ...
  const [result, setResult] = useState<{ shopSlug: string; shopName: string; passcode: string } | null>(null);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setResult({ shopSlug: data.shopSlug, shopName: data.shopName, passcode: data.passcode });
    setState('done');
  }
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0A0A0A', padding: '1rem' }}>
        <div style={{ width: '100%', maxWidth: 420, textAlign: 'center' }}>
          ...
          <div style={{
            ...
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.75rem' }}>
              Your passcode — this is your login
            </p>
            <p style={{
              fontFamily: 'monospace', fontSize: '2.25rem', fontWeight: 700,
              letterSpacing: '0.3em', color: '#C8F135', margin: 0,
            }}>
              {result.passcode}
            </p>
            <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.75rem', marginTop: '0.75rem' }}>
              No password, no email. Save this or add your card to Wallet below.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxWidth: 320, margin: '0 auto' }}>
            ...
            <button
              onClick={() => router.push('/owner/login?welcome=1')}
              className="btn-lime"
            ...
            >
              Sign in with my passcode <ArrowRight size={16} />
            </button>
          </div>
        </div>
      </div>
